...
<template>
  <div class="platform-wrap">
        ...
        <table  cellspacing="0" cellpadding="0" border="0" width="100%">
          <thead>
            <tr>
              ...
              <th class="is-leaf" v-if="filterTableNameCellIsVisible" @click="addCategory(rowName)">{{rowName}}&nbsp;&nbsp;<i class="el-icon-plus"></i></th>
              <th class="is-leaf" v-if="filterTableTagCellIsVisible" @click="addTag">标签&nbsp;&nbsp;<i class="el-icon-plus"></i></th>
    ...
    <el-dialog title="变更" v-model="dialogFormVisible" size="tiny" :close-on-click-modal="false" class="tiny-dialog">
      <div class="dialog-box">
        <el-form :model="dataUpdate">
          <el-form-item>
            <el-input v-model="dataUpdate.name" placeholder="请输入内容" :maxlength="12"></el-input>
          </el-form-item>
          <div class="ft-group">
    ...
    <!-- 新增学段，学科，年级 -->
    <el-dialog title="新增" v-model="addFormVisible" size="tiny" :close-on-click-modal="false" class="tiny-dialog">
      <div class="dialog-box">
        <el-form>
          <el-form-item>
          ...
          <div class="ft-group">
            <button class="el-button el-button--primary" @click="addCategoryForm">提交</button>
            <button class="el-button el-button--primary" @click="addFormVisible = false">取消</button>
          </div>
        </el-form>
      </div>
    </el-dialog>
  ...
  export default {
    ...
    data () {
      return {
        ...
        addFormVisible: false, // 新增学段，学科，年级
        phasegradeList: [], // 学段年级数据
        subjectList: [], // 学科数据
        addSelectValue: '',
        addSelectData: [],
    ...
    methods: {
      ...mapActions([CHANGE_PENDING, GET_CATEGORY_LIST, UPDATE_CATEGORY, ADD_CATEGORY, DEL_CATEGORY, ADD_TAGS, DEL_CATEGORY_TAG,
        BASIC_DATA_PHASE_GRADE_GET_LIST, BASIC_DATA_SUBJECT_GET_LIST]),
      ...
      openUpdateDialog (index, rowData) {
        let type = this.rowName2typeCode(this.rowName)
        // type 1学段，2学科，3版本，4年级
        if (type == 1 || type == 2 || type == 4) {
          this.$message.error('此类别不允许变更')
          return
        }
        if (rowData.tagId) {
          // 更新标签
      ...
      addCategory (text) {
        ...
        if (type == 1 || type == 2 || type == 4) {
          this.addSelectValue = ''
          if (type == 1) {
            this.addSelectData = this.phasegradeList
          } else if (type == 2) {
            this.addSelectData = this.subjectList.map((item) => {
              return {
            ...
            let grades = []
            this.phasegradeList.map((item) => {
              if (this.sectionValue.name == item.name) {
                grades = item.grades
              }
            })
            this.addSelectData = grades.map((item) => {
              return {
                name: item.gradeName
              }
            })
      ...
      addCategoryForm () {
        if (!this.addSelectValue) {
          this.$message.error('请选择再提交')
          return
        }
        let type = this.rowName2typeCode(this.rowName)
        let data = {
          parentCateId: this.cateId,
          name: this.addSelectValue,
          type: type
        }
        this.submitCategory(data, type)
      },
       ...
       * 获取学段年级列表
       */
      getPhasegradeListList () {
        this.CHANGE_PENDING(true)
        this.BASIC_DATA_PHASE_GRADE_GET_LIST().then((res) => {
          this.CHANGE_PENDING(false)
          if (res.code === SUCCESS) {
            this.phasegradeList = res.data
          } else {
            // 异常错误处理
            this.$message.error('获取学段年级列表,请尝试刷新')
          }
        }).catch((err) => {
          globalErrorPrint(err)
          this.CHANGE_PENDING(false)
      ...
      getSubjectList () {
        ...
        this.BASIC_DATA_SUBJECT_GET_LIST().then((res) => {
          ...
          if (res.code === SUCCESS) {
            ...
            // 异常错误处理
            this.$message.error('获取学科列表,请尝试刷新')
          }
        }).catch((err) => {
          globalErrorPrint(err)
          this.CHANGE_PENDING(false)
